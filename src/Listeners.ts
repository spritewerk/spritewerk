...
import { Handler, HandlerObject, HandlerObjects, ListenerTypes } from "./types";
...
export default class Listeners {
  ...
  public handleEvents (events:Events):void {
    const queuedEvents = events.getQueuedEvents();
  ...
  public remove (target:Sprite, type:ListenerTypes, handler:Handler):void {
    const handlerObjects = this.handlerObjects[type];
